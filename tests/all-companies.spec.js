...
test.describe('All Companies', () => {
  ...
  companies.forEach(company => {
    test(`can start ${company.name} challenge`, async ({ page }) => {
      ...
      await expect(page.getByRole('heading', { name: `${company.name} Architecture Challenge` })).toBeVisible();
    ...
    test(`${company.name} displays correct difficulty`, async ({ page }) => {
      await expect(page.getByText(company.difficulty)).toBeVisible();
    });
  ...
  test('all companies have unique challenges per phase', async ({ page }) => {
    ...
    const netflixChallenge = await page.getByText(/Netflix started as a DVD-by-mail service/).textContent();
    
    await page.getByRole('button', { name: '← Back to Menu' }).click();
    await page.getByRole('button', { name: /Slack/ }).click();
    const slackChallenge = await page.textContent('[data-testid="challenge-text"], .card:has-text("Challenge") p');
  ...
  test('phase progression works for all companies', async ({ page }) => {
    for (const company of companies.slice(0, 2)) { // Test first 2 companies to save time
      await page.goto('/');
      await page.getByRole('button', { name: new RegExp(company.name) }).click();
